...
function drawEmoji(canvas, img, face) {
  ...
  // <your code here>
  // console.log(face.featurePoints[0].x, face.featurePoints[0].y);
  ctx.font = '48px serif'
  ctx.strokeText(face.emojis.dominantEmoji, face.featurePoints[4].x, face.featurePoints[4].y);
...
function playGame(canvas, img, face) {
    ...
    if (score === 3){
        onStop();
        setTargetEmoji("");
        document.getElementById("target").innerHTML = "You Win!";
    }
    else if (toUnicode(face.emojis.dominantEmoji) == toUnicode(target)){
        setTargetEmoji(emojis[Math.floor(Math.random()*emojis.length)]);
        setScore(score+1, 3);
    }
